...
@section('title', 'Case Hospital Level')
...
<div class="container">
    ...
    <div class="row">
        <div class="col-xl-12 pa-0">
            ...
            @include('dispatchers.single.casenav', ['active' => 'hospital-level'])
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <section class="hk-sec-wrapper">
                                <h5 class="hk-sec-title">Hospital Level</h5>
                                <form method="POST" action="{{route('cases.dispatch.store.hospital-level', $case)}}" role="form">
                                    @csrf
                                    <div class="form-group">
                                        <label for="hospital_level">Level of Hospital</label>
                                        <select name="hospital_level" id="hospital_level" class="form-control @error('hospital_level') is-invalid @enderror" required="">
                                            <option value="">Select Level</option>
                                            @foreach($facilities as $item)
                                            <option value="{{ $item->keph_level }}" {{ $item->keph_level==$case->hospital_level?'selected':'' }}>{{ $item->keph_level }}</option>
                                            @endforeach
                                        </select>
